...
import { randomiseNumbers } from '../../src/ui/paramControls';

describe('randomising the parameters', () => {
  it('stays inside every declared range and on the declared step', () => {
    for (let seed = 1; seed <= 200; seed++) {
      const got = randomiseNumbers(polyarcViz.params, seed);
      for (const spec of polyarcViz.params) {
        if (spec.kind !== 'number') continue;
        const v = got[spec.id] as number;
        expect(v, `${spec.id} below min at seed ${seed}`).toBeGreaterThanOrEqual(spec.min);
        expect(v, `${spec.id} above max at seed ${seed}`).toBeLessThanOrEqual(spec.max);
        expect((v - spec.min) % spec.step, `${spec.id} off-step at seed ${seed}`).toBeCloseTo(0, 9);
      }
    }
  });

  it('gives a different draw for a different seed, and the same for the same', () => {
    expect(randomiseNumbers(polyarcViz.params, 1)).toEqual(randomiseNumbers(polyarcViz.params, 1));
    expect(randomiseNumbers(polyarcViz.params, 1)).not.toEqual(randomiseNumbers(polyarcViz.params, 2));
  });

  it('leaves selects and booleans alone', () => {
    // Randomising a colour mode or a scale type would change what the picture
    // is, not where it sits in a continuum.
    const specs = [
      { kind: 'number' as const, id: 'n', label: 'n', default: 1, min: 1, max: 10, step: 1 },
      { kind: 'select' as const, id: 's', label: 's', default: 'a', options: ['a', 'b'] },
      { kind: 'boolean' as const, id: 'b', label: 'b', default: true },
    ];
    expect(Object.keys(randomiseNumbers(specs, 7))).toEqual(['n']);
  });

  it('actually explores the range rather than clustering', () => {
    // A draw that always landed near one end would look random and be useless.
    const seen = new Set<number>();
    for (let seed = 1; seed <= 300; seed++) seen.add(randomiseNumbers(polyarcViz.params, seed).modulus as number);
